...
TEST(BinaryTranslatorE2E, DescriptorPrologueRedirectsEntryWithoutOverwritingOriginalEntry) {
  ...
  EXPECT_EQ(translated_info->guest_vgpr_count, original_info->target_vgpr_count)
      << "Descriptor translation should not reserve speculative VGPR headroom when semantic "
         "lowerings did not request any";
