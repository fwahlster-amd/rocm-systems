...
constexpr size_t op_size{4096};
constexpr size_t op_count{4};
constexpr size_t file_size{op_size * op_count};
...
TEST_F(BatchTest, BatchReadMultipleOperations)
{
    ...
    setupBatch(op_count);
    std::array<hipFileIOParams_t, op_count> ops{};
    for (size_t i = 0; i < ops.size(); ++i) {
        ops[i] = makeOp(i, hipFileBatchRead);
    }
    ASSERT_EQ(hipFileBatchIOSubmit(batch_handle, ops.size(), ops.data(), 0), HIPFILE_SUCCESS);
