...
ncclGin_t* getNcclGin_v11_internal(ncclGin_v11_t* ncclGin_v11) {
  if ((ncclGin_built_in_idx >= ncclGin_built_in_count) || !ncclGin_v11) {
    return nullptr;
  }

  ncclGin_t* __ncclGin = &ncclGin_built_in[ncclGin_built_in_idx++];
  __ncclGin->name = ncclGin_v11->name;
  __ncclGin->init = ncclGin_v11->init;
  __ncclGin->devices = ncclGin_v11->devices;
  __ncclGin->getProperties = ncclGin_getProperties;
  __ncclGin->listen = ncclGin_v11->listen;
  __ncclGin->connect = ncclGin_v11->connect;
  __ncclGin->createContext = ncclGin_createContext;
  __ncclGin->regMrSym = ncclGin_v11->regMrSym;
  __ncclGin->regMrSymDmaBuf = ncclGin_v11->regMrSymDmaBuf;
  __ncclGin->deregMrSym = ncclGin_v11->deregMrSym;
  __ncclGin->destroyContext = ncclGin_destroyContext;
  __ncclGin->closeColl = ncclGin_v11->closeColl;
  __ncclGin->closeListen = ncclGin_v11->closeListen;
  __ncclGin->iput = ncclGin_iput;
  __ncclGin->iputSignal = ncclGin_iputSignal;
  __ncclGin->iget = NULL;
  __ncclGin->iflush = ncclGin_iflush;
  __ncclGin->test = ncclGin_v11->test;
  __ncclGin->ginProgress = ncclGin_v11->ginProgress;
  __ncclGin->queryLastError = ncclGin_v11->queryLastError;
  __ncclGin->finalize = ncclGin_v11->finalize;
  return __ncclGin;
}
