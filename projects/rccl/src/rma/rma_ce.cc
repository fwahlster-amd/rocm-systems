...
ncclResult_t ncclRmaCeInit(struct ncclComm* comm){
  ...
  for (int i = 0; i < comm->rmaState.rmaCeState.rmaCeCtxCount; i++) {
    ...
    // RCCL: AICOMNET-196 — passing comm->memManager omitted until RCCL alloc.h gains a manager-aware overload.
    NCCLCHECKGOTO(ncclCudaCalloc(&ceCtx->signalConstDev, 2), ret, fail);
...
ncclResult_t ncclRmaCeFinalize(struct ncclComm* comm){
  ...
  for (int i = 0; i < comm->rmaState.rmaCeState.rmaCeCtxCount; i++) {
    ...
    if (ceCtx->signalConstDev) NCCLCHECKGOTO(ncclCudaFree(ceCtx->signalConstDev), ret, fail);
