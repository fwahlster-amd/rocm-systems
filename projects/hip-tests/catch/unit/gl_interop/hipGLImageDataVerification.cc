...
HIP_TEST_CASE(Unit_GLHIPImageData_Positive_Mipmap_HIPWrite_GLRead_HigherLevels) {
  ...
  /* Verify base level was not modified. */
  std::vector<uint32_t> gl_base = read_gl_texture(tex, kWidth, kHeight);
  for (unsigned int i = 0; i < kNumPixels; ++i) {
    REQUIRE(gl_base[i] == fill_color[0]);
  }
...
/*
 * Round-trip for mip level 1 (32×32): upload a gradient to GL at level 1,
 * map it in HIP, run the invert kernel (all four channels: 255 - value),
 * unmap, read back from GL at level 1, and verify the per-channel complement.
 */
HIP_TEST_CASE(Unit_GLHIPImageData_Positive_Mipmap_RoundTrip_InvertLevel1) {
  CHECK_IMAGE_SUPPORT;
  GLContextScopeGuard gl_context;

  const int kTargetLevel = 1;
  ...
  /* Level 2: arbitrary filler so the texture is complete. */
  unsigned int w2 = mip_dim(kWidth, 2), h2 = mip_dim(kHeight, 2);
  std::vector<uint32_t> mip2(w2 * h2, pack_rgba(0xAA, 0xBB, 0xCC, 0xFF));
  upload_mip_level(tex, 2, w2, h2, mip2);
